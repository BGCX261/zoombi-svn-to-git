...
class ZLoader extends ZObject implements IZSingleton
{
    ...
    private function & _loadClass( $a_name, & $a_instances, $a_prefix, $a_baseclass )
    {
        ...
        $m_prefix = ucfirst( trim((string)$c_prefix) );
		...
		$class_file	= Zoombi::config($c_prefix.'.file_prefix') .
            $class_name .
            Zoombi::config($c_prefix.'.file_suffix', $m_prefix) .
            '.' .
        ...
        $instance = new $class_class;
        $instance->setName($class_name);
    ...
    static public final function _loadFile( $a_name, $a_section, $a_title = 'ZLoader' )
    {
        $file_base = null;
        ...
        $file_file = Zoombi::config( $a_section.'.file_prefix') .
            $file_name .
            Zoombi::config($a_section.'.file_suffix', $section) .
            '.' .
    ...
    static public final function & controller( $a_controller )
	{
        ...
        try
        {
            $c = self::getInstance()->_loadClass( $a_controller, self::getInstance()->m_controllers, 'controller', 'ZController' );
        }
        catch ( Exception $e )
        {
	...
	static public final function & model( $a_model )
	{
        ...
        try
        {
            $c = self::getInstance()->_loadClass( $a_model, self::getInstance()->m_models, 'model', 'ZModel' );
        }
        catch ( Exception $e )
        {
     ...
     * @return ZPlugin
     */
    static public final function & plugin( $a_plugin )
	{
        $c = null;
        try
        {
            $c = self::getInstance()->_loadClass( $a_plugin, $c, 'plugin', 'ZPlugin' );
        }
        catch ( Exception $e )
        {
    ...
    static public final function action( $a_action )
    {
        ...
        try
        {
            $c = self::getInstance()->_loadClass( $a_action, $c, 'action', 'ZAction' );
        }
        catch ( Exception $e )
        {
    ...
    static public final function view( $a_view )
    {
        ...
        try
        {
            $ret = self::getInstance()->_loadFile($a_view, 'view', 'ZView');
        }
        catch ( Exception $e )
        {
	...
	static public final function helper( $a_helper )
	{
        try
        {
			$filename = self::getInstance()->_loadFile($a_helper, 'helper', 'Helper');
        }
        catch ( Exception $e )
        {
