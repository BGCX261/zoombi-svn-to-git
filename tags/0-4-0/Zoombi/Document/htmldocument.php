...
class ZHtmlDocument extends ZXmlDocument
{
	...
	public function & cssAddFile( $a_style, $a_media = 'all' )
	{
		switch( gettype($a_style) )
		{
			case 'string':
				$this->m_styles['include'][] = array($a_style,$a_media);
				break;

			case 'array':
				foreach( $a_style as $s )
					$this->cssAddFile( $s, $a_media );
				break;
		}
        return $this;
	}

	public function & cssAddUrl( $a_style, $a_media = 'all' )
	{
		switch( gettype($a_style) )
		{
			case 'string':
				$this->m_styles['url'][] = array($a_style,$a_media);
				break;

			case 'array':
				foreach( $a_style as $s )
					$this->cssAddUrl( $s, $a_media );
				break;
		}
        return $this;
	}
	...
	public function compile()
	{
		...
		$d->normalizeDocument();
		return str_replace($placeholder, $this->body(), $d->saveHtml(/*null,/*LIBXML_NOEMPTYTAG|LIBXML_NONET|LIBXML_NOXMLDECL|LIBXML_DTDVALID|LIBXML_NOCDATA*/));
    }
