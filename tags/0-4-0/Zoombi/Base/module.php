...
class ZModule extends ZApplicationObject
{
	...
	{
		return $this->fromBaseDir($this->getConfig()->getValue('path.model', self::DEFAULT_MODEL_DIR));
	}
	...
	{
		return $this->fromBaseDir($this->getConfig()->getValue('path.view', self::DEFAULT_CONTROLLER_DIR));
	}
	...
	{
		return $this->fromBaseDir($this->getConfig()->getValue('path.plugin', self::DEFAULT_PLUGIN_DIR));
	}
	...
	{
		return $this->fromBaseDir($this->getConfig()->getValue('path.action', self::DEFAULT_ACTION_DIR));
	}
	...
	{
		return $this->fromBaseDir($this->getConfig()->getValue('path.helper', self::DEFAULT_HELPER_DIR));
	}
	...
	{
		return $this->fromBaseDir($this->getConfig()->getValue('path.module', Zoombi::DEFAULT_MODULE_DIR));
	}
	...
	{
		return $this->fromBaseDir($this->getConfig()->getValue('path.config', self::DEFAULT_CONFIG_DIR));
	}
	...
	public final function & route( $a_path, $a_args = array( ) )
	{
		...
		$c = /*$this->getRoute()*/$route->getController();
		$a = /*$this->getRoute()*/$route->getAction();
		...
		$this/*->push()*/->setRoute($route)->setArgs($a_args);
	...
	{
		$this->route($a_path, $a_args)->outputFlush();
