...
abstract class ZController extends ZObject
{
	...
	{
		$pref = $this->application->getConfig()->getValue('controller.action_prefix');
		$suff = $this->application->getConfig()->getValue('controller.action_suffix');
	...
	public function render( $a_view = null, $a_data = null, $a_return = false )
	{
		...
		$v = new ZView($this, $a_view);
		$o = $this->renderView($v, $a_data, $a_return);
		unset($v);
		return $o;
	}
	...
	public function referer( $a_params = false )
	{
		...
		return $this->application->fromBaseUrl(
			( $a_params ? $route : $route->getController() . Zoombi::US . $route->getAction() )
		);
	}
