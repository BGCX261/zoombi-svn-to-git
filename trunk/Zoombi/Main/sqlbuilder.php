...
class Zoombi_SqlBuilder
{
	...
	public function & where( $arg1, $arg2 = null, $arg3 = null )
	{
		switch(func_num_args())
		{
			...
			case 3:
				$this->_where($this->_make_expression(array($arg1, $arg2, $arg3)), 'AND');
				break;
		}
		return $this;
	}

	public function & or_where( $arg1, $arg2 = null, $arg3 = null )
	{
		switch(func_num_args())
		{
			case 1:
			...
			case 2:
				$this->_where($this->_make_expression($arg1, $arg2), 'OR');
				break;

			case 3:
				$this->_where($this->_make_expression(array($arg1, $arg2, $arg3)), 'OR');
				break;
		}
		return $this;
	}
	...
	public function & having( $a_key, $a_operator = null, $a_value = null )
	{
		switch(func_num_args())
		{
			...
			case 2:
				$a = func_get_arg(0);
				$b = func_get_arg(1);
				$this->_having($this->_make_expression($a, $b), 'AND');
				break;

			case 3:
				$a = func_get_args();
				$this->_having($this->_make_expression($a), 'AND');
				break;
		}
		return $this;
	}
	...
	private static $m_expr_pattern = '/`?(\w+(?:(?:\.\w+)+)?)`?\s*(!=|>=|<=|==|=|>|<)\s*(.*)/i';
	private static $m_expr_operator = '/`?(\w+(?:(?:\.\w+)+)?)`?\s+(OR|AND|XOR|MOD|LIKE|RLIKE|REGEXP|NOT|IS|IN|AS|BYNARY|CASE|IS NOT|INTERVAL)\s./i';
	private static $m_expr_function = '/^\s*[A-Z_]{2,20}+\(.*\)/';
	private static $m_expr_group = '/^\(.*\)/';

	public function _is_expression_function( $a_test )
	{
		return preg_match(self::$m_expr_function, (string)$a_test) === 1;
	}

	public function _has_expression( $a_test )
	{
		return preg_match(self::$m_expr_pattern, (string)$a_test) === 1;
	}

	public function _get_expression( $a_exp )
	{
		$matches = array();
		$c = preg_match(self::$m_expr_pattern, (string)$a_exp, $matches);
		if($c)
			array_shift($matches);
		else
		{
			$c = preg_match(self::$m_expr_operator, (string)$a_exp, $matches);
			if($c)
				array_shift($matches);
		}
	...
	public function _get_expression_key( $a_exp )
	{
		$e = self::_get_expression($a_exp);
		if($e)
			return $e[0];
	}

	public function _get_expression_operator( $a_exp )
	{
		$e = self::_get_expression($a_exp);
		if($e)
			return $e[1];
	...
	public function _get_expression_values( $a_exp )
	{
		$e = self::_get_expression($a_exp);
		if($e)
			return $e[2];
	...
	public function _make_value( $a_value )
	{
			...
			case 'string':
				switch(strtolower($a_value))
				{
					default:
						...
						{
							if(self::_is_expression_function($a_value))
								return $a_value;
							else
							{
	...
	public function _make_key( $a_key )
	{
		$k = strval($a_key);
		$s = $k{0};
		$e = $k{strlen($k) - 1};
	...
	public function _make_expression( $arg1, $arg2 = null, $arg3 = null )
	{
		switch(func_num_args())
		{
			case 1:
				...
				{
					if( count($arg1) == 3 )
						return self::_make_expression($arg1[0], $arg1[1], $arg1[2]);
					
					if( count($arg1) == 2 )
						return self::_make_expression($arg1[0], $arg1[1]);
					
					if( count($arg1) == 1 )
						return self::_make_expression($arg1[0]);
				}
				return $arg1 . '';
			...
			case 2:
				if( is_array($arg2) )
				{
					$_v = array();
					foreach ( $arg2 as $_)
						$_v[] = $this->_make_value($_);
						
					return self::_make_key($arg1) . ' IN(' . implode(',',$_v).')';
				}
				return self::_make_expression($arg1, '=', $arg2);

			case 3:
				return self::_make_key($arg1) . $arg2 . self::_make_value($arg3);
		}
	}

	public function _fix_expression( $a_exp )
	{
		return self::_make_expression(self::_get_expression($a_exp));
	}
