 ...
 * MVC (View)
 */
class Zoombi_View extends Zoombi_Object
{
	...
	/**
	 * Constructor
	 * @param Zoombi_Controller $a_controller
	 * @param string $a_view
	 */
	...
	protected function renderFile()
	{
		...
		if(empty($___file___))
		{
			if($this->application->isMode(Zoombi_Application::MODE_DEBUG))
				throw new Zoombi_Exception_View('View file empty.', Zoombi_Exception_View::EXC_EMPTY);
			return;
		}
		...
		{
			if($this->application->isMode(Zoombi_Application::MODE_DEBUG))
				throw new Zoombi_Exception_View('View file "' . $___file___ . '" is not found.', Zoombi_Exception_View::EXC_NOFILE);
			return;
		}
		...
		{
			if($this->application->isMode(Zoombi_Application::MODE_DEBUG))
				throw new Zoombi_Exception_View('Path "' . $___file___ . '" is not file.', Zoombi_Exception_View::EXC_NOFILE);
			return;
		}
		...
		{
			if($this->application->isMode(Zoombi_Application::MODE_DEBUG))
				throw new Zoombi_Exception_View('View file "' . $___file___ . '" is not readable.', Zoombi_Exception_View::EXC_NOREAD);
			return;
		}
