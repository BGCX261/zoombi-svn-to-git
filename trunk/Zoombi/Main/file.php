...
class Zoombi_File
{
	...
	static public function dir( $a_filepath = null )
	{
		$info = isset($this) ? $this->info() : self::info($a_filepath);
		return $info['dirname'];
	}
	
	static public function dirname( $a_filepath = null )
	{
		$info = isset($this) ? $this->info() : self::info($a_filepath);
		return $info['dirname'];
	}
	
	static public function base( $a_filepath = null )
	{
		$info = isset($this) ? $this->info() : self::info($a_filepath);		
		return $info['basename'];
	}
	
	static public function basename( $a_filepath = null )
	{
		$info = isset($this) ? $this->info() : self::info($a_filepath);		
		return $info['basename'];
	}
	...
	/**
	 * File info
	 * @param string $a_filepath
	 * @return array
	 */
	static public function info( $a_filepath = null )
	{
		return isset($this) ? pathinfo($this->m_filename) : pathinfo($a_filepath);
	}
	
	static public function ext( $a_filepath = null )
	{
		$info = isset($this) ? $this->info() : self::info($a_filepath);		
		return $info['extension'];
	}
	
	static public function extension( $a_filepath = null )
	{
		$info = isset($this) ? $this->info() : self::info($a_filepath);		
		return $info['extension'];
	}
