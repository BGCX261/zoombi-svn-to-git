...
class Zoombi_Rss_Item
{
	...
	{
		$this->m_author = new Zoombi_Rss_Author($a_value);
	}
	...
	public final function setSource( $a_source )
	{
		...
		if(is_a($a_source, 'Zoombi_Rss_Source'))
			$this->m_sourse = $a_source;

		if(is_array($a_source))
			$this->m_sourse = new Zoombi_Rss_Source($a_source);
	}

	public final function addEnclosure( $a_value )
	{
		if(is_a($a_value, 'Zoombi_Rss_Enclosure'))
		{
		...
		if(is_array($a_value))
		{
			$this->m_enclosures[] = new Zoombi_Rss_Enclosure($a_value);
		}
	}
